    ...
    <div class="card">
      ...
      <div v-if="matrix.length" class="list">
        <div class="card" v-for="r in matrix" :key="r.betId" style="padding:12px">
          ...
          <div class="list">
            <div v-for="(choice, name) in r.answers" :key="name" style="display:flex;justify-content:space-between">
              <div>{{name}}</div><div>{{choice}}</div>
            </div>
          </div>
        </div>
      </div>
...
async function closeEvent(){
  try{ await fetch(`${import.meta.env.VITE_API_BASE}/events/${eventId}/close?sig=${encodeURIComponent(sig)}`, { method:'POST' }) ; await refresh() }
  catch(e:any){ error.value = e.message }
}
