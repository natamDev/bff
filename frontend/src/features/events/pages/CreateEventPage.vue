...
const res = ref<null | { eventId:string; hostLink:string; inviteLinks:Record<string,string> }>(null)
const error = ref('')
...
async function create(){
  ...
  try{
    ...
    const r = await store.create({ title: title.value, description: description.value, place: place.value, startAt: startAt.value, endAt: endAt.value, invites })
    res.value = r
