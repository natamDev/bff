...
public class EventResource {
  ...
  @POST @Path("/{eventId}/bets") @Transactional
  ...
  @PATCH @Path("/{eventId}/bets/{betId}") @Transactional
  public EventView setBetStatus(@PathParam("eventId") String eventId, @PathParam("betId") String betId, @QueryParam("sig") String sig, BetUpdateReq req) {
    ...
    switch (req.status) { case "open" -> bet.status = Event.Bet.Status.open; case "true" -> bet.status = Event.Bet.Status._true; case "false" -> bet.status = Event.Bet.Status._false; default -> throw new BadRequestException("status invalid"); }
    repo.update(e);
    return EventView.of(e);
  }

  @POST @Path("/{eventId}/bets/{betId}/predict") @Transactional
  public EventView predict(@PathParam("eventId") String eventId, @PathParam("betId") String betId,
    ...
    var choice = switch (req.choice) { case "YES" -> Event.Prediction.Choice.YES; case "NO" -> Event.Prediction.Choice.NO; default -> throw new BadRequestException("choice must be YES or NO"); };
    var existing = bet.predictions.stream().filter(p -> p.inviteId.equals(inviteId)).findFirst();
    if(existing.isPresent()){ existing.get().choice = choice; existing.get().at = OffsetDateTime.now(); }
    else { Event.Prediction p = new Event.Prediction(); p.inviteId = inviteId; p.choice = choice; p.at = OffsetDateTime.now(); bet.predictions.add(p); }
    repo.update(e);
    return EventView.of(e);
  }
  ...
  @POST @Path("/{eventId}/close") @Transactional
  public EventView close(@PathParam("eventId") String eventId, @QueryParam("sig") String sig) {
    Event e = repo.byId(new ObjectId(eventId));
    if(e==null) throw new NotFoundException();
    if(!links.verifyHost(eventId, sig, e.hostSecret)) throw new ForbiddenException();
    e.closed = true; repo.update(e); return EventView.of(e);
  }

  // Views
  @JsonInclude(JsonInclude.Include.NON_NULL)
  public static class EventView {
    public String id, title, description, place, startAt, endAt; public boolean closed; public List<BetView> bets;
    static EventView of(Event e){
      ...
      v.startAt = e.startAt!=null? e.startAt.toString(): null; v.endAt = e.endAt!=null? e.endAt.toString(): null;
      v.closed = e.closed; v.bets = e.bets.stream().map(BetView::of).toList(); return v;
    }
  }
  public static class BetView { public String id, text, status;
    static BetView of(Event.Bet b){ BetView v = new BetView(); v.id=b.id; v.text=b.text; v.status=switch(b.status){case open->"open";case _true->"true";case _false->"false";}; return v; } }
  public static class InviteEventView { public String id, title, description, place, startAt, endAt; public boolean closed; public List<InviteBetRow> bets;
    static InviteEventView of(Event e, String inviteId){
      InviteEventView v = new InviteEventView();
      v.id=e.id.toHexString(); v.title=e.title; v.description=e.description; v.place=e.place;
      v.startAt = e.startAt!=null? e.startAt.toString(): null; v.endAt = e.endAt!=null? e.endAt.toString(): null; v.closed=e.closed;
      v.bets = e.bets.stream().map(b->{ InviteBetRow r = new InviteBetRow(); r.id=b.id; r.text=b.text; r.status=switch(b.status){case open->"open";case _true->"true";case _false->"false";}; r.myChoice=b.predictions.stream().filter(p->p.inviteId.equals(inviteId)).map(p->p.choice.name()).findFirst().orElse(null); return r; }).toList(); return v;
    } }
  public static class InviteBetRow { public String id, text, status, myChoice; }
  public static class MatrixRow { public String betId, text, status; public Map<String,String> answers; }
}
